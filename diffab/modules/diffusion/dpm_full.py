...
class EpsilonNetwithRAG(nn.Module):
    ...
    def __init__(self, res_feat_dim, pair_feat_dim, num_layers, encoder_opt={}):
        ...
        #     nn.Lambda(lambda x: x.mean(dim=1))  # 对 dim=1 进行平均池化
        # )

        self.prompt_attn = nn.MultiheadAttention(embed_dim=res_feat_dim, num_heads=8,batch_first=True)

    def forward(self, v_t, p_t, s_t, res_feat, pair_feat, beta, mask_generate, mask_res, feature_directory, topnheavy=None, topnlight=None):
        N, L = mask_res.size()
        R = so3vec_to_rotation(v_t)  # (N, L, 3, 3)
        ...
        c_denoised = self.eps_seq_net(in_feat)  # 已经过 softmax，(N, L, 20)

        return v_next, R_next, eps_pos, c_denoised

    

class FullDPMwithRAG(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.eps_net = EpsilonNetwithRAG(res_feat_dim, pair_feat_dim, **eps_net_opt)
        self.num_steps = num_steps
        self.trans_rot = RotationTransition(num_steps, **trans_rot_opt)#加噪声模块
        self.trans_pos = PositionTransition(num_steps, **trans_pos_opt)
        self.trans_seq = AminoacidCategoricalTransition(num_steps, **trans_seq_opt)
    ...
    def forward(self, v_0, p_0, s_0, res_feat, pair_feat, mask_generate, mask_res, denoise_structure, denoise_sequence, t=None,topnheavy=None,topnlight=None,feature_directory=None):
        N, L = res_feat.shape[:2]
        if t == None:
            t = torch.randint(0, self.num_steps, (N,), dtype=torch.long, device=self._dummy.device)
        p_0 = self._normalize_position(p_0)
        ...
        beta = self.trans_pos.var_sched.betas[t]
        v_pred, R_pred, eps_p_pred, c_denoised = self.eps_net(
            v_noisy, p_noisy, s_noisy, res_feat, pair_feat, beta, mask_generate, mask_res,topnheavy=topnheavy,topnlight=topnlight,feature_directory=feature_directory
        )   # (N, L, 3), (N, L, 3, 3), (N, L, 3), (N, L, 20), (N, L)
...
class FullDPMwithRAGcal(nn.Module):
