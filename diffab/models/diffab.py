...
from diffab.modules.diffusion.dpm_full import FullDPM,FullDPMwithRAG,FullDPMwithRAGcal
...
@register_model('ragdiffab')
class DiffusionAntibodyDesignwithRAG(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.residue_embed = ResidueEmbedding(cfg.res_feat_dim, num_atoms)
        self.pair_embed = PairEmbedding(cfg.pair_feat_dim, num_atoms)
        self.feature_directory=cfg.get('feature_directory', None)
        

        self.diffusion = FullDPMwithRAG(
            cfg.res_feat_dim,
            cfg.pair_feat_dim,
            **cfg.diffusion,
        )
    ...
    def forward(self, batch):
        mask_generate = batch['generate_flag']
        mask_res = batch['mask']
        topnheavy=batch['topn_heavy']
        topnlight=batch['topn_light']
        res_feat, pair_feat, R_0, p_0 = self.encode(
            batch,
            remove_structure = self.cfg.get('train_structure', True),
            remove_sequence = self.cfg.get('train_sequence', True)
...
@register_model('ragdiffabcal')
class DiffusionAntibodyDesignwithRAGcal(nn.Module):
